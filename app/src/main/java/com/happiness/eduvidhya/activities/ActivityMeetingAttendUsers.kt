...
import com.happiness.eduvidhya.adapters.AdapterAllUsersAttendence
...
class ActivityMeetingAttendUsers : AppCompatActivity() {
    ...
    var strMeetingID:String ?= null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_meeting_attend_users)

        back_top_bar_img = findViewById(R.id.back_top_bar_img)
        title_top_bar_txt = findViewById(R.id.title_top_bar_txt)
        title_top_bar_txt.setText("All Users Attendence")
        back_top_bar_img.setOnClickListener {
            finish()
        }

        strMeetingID = intent.getStringExtra("Email")


        val layoutManager = LinearLayoutManager(this@ActivityMeetingAttendUsers)
    ...
    private var mRecyclerAdapter: AdapterAllUsersAttendence? = null
    ...
    fun mFaculties() {
        ...
        if (Constant.hasNetworkAvailable(applicationContext)) {
            ...
            faculties_collection.document(email.toString()).collection("MeetingHistory").document(strMeetingID.toString()).collection("Users").get()
                    ...
                    override fun onComplete(@NonNull task: Task<QuerySnapshot>) {
                        ...
                        if (task.isSuccessful()) {
                            ...
                            if (users_faculty!!.size == 0)
                            {
                                ...
                                mRecyclerAdapter = AdapterAllUsersAttendence(
                                    this@ActivityMeetingAttendUsers,
                                    users_faculty)
